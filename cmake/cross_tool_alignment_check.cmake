...
foreach(_f ${_tool_gui_files})
    if(NOT _f IN_LIST _dl_gui_files)
        list(APPEND _alignment_errors "GIS_TOOL ONLY (not in GIS_DL_TOOL): src/gui/${_f}")
    endif()
endforeach()

foreach(_f ${_dl_gui_files})
    if(NOT _f IN_LIST _tool_gui_files)
        list(APPEND _alignment_errors "GIS_DL_TOOL ONLY (not in GIS_TOOL): src/gui/${_f}")
    endif()
endforeach()

add_test(
    NAME cross_tool_alignment_check
    COMMAND ${CMAKE_COMMAND} -E echo "Cross-tool alignment check: ${_alignment_errors}"
)

if(_alignment_errors)
    message(WARNING "Cross-tool alignment issues detected:\n${_alignment_errors}")
