if(NOT DEFINED GUI_PATH OR NOT DEFINED INPUT_PATH OR NOT DEFINED OUTPUT_PATH OR NOT DEFINED SCREENSHOT_PATH)
    message(FATAL_ERROR "Missing required GUI invalid-param regression arguments.")
endif()

include("${CMAKE_CURRENT_LIST_DIR}/gui_regression_helpers.cmake")

get_filename_component(OUTPUT_DIR "${OUTPUT_PATH}" DIRECTORY)
set(STATUS_PATH "${OUTPUT_DIR}/status.json")
gis_gui_prepare_artifact_paths("${OUTPUT_PATH}" "${SCREENSHOT_PATH}" "${STATUS_PATH}")

execute_process(
    COMMAND "${GUI_PATH}"
        -platform offscreen
        --select-plugin segment
        --select-action segment_raster
        --set-param "model_path=test_data/models/test_seg_model.onnx"
        --set-param "input_raster=${INPUT_PATH}"
        --set-param "output_tif=${OUTPUT_PATH}"
        --set-param "bogus=1"
        --auto-execute
        --quit-on-finish
        --screenshot "${SCREENSHOT_PATH}"
        --status-file "${STATUS_PATH}"
    WORKING_DIRECTORY "${CMAKE_CURRENT_LIST_DIR}/.."
    RESULT_VARIABLE GUI_EXIT_CODE
    OUTPUT_VARIABLE GUI_STDOUT
    ERROR_VARIABLE GUI_STDERR
)

if(NOT "${GUI_EXIT_CODE}" STREQUAL "2")
    message(FATAL_ERROR
        "GUI invalid-param regression returned unexpected exit code ${GUI_EXIT_CODE}\n"
        "stdout:\n${GUI_STDOUT}\n"
        "stderr:\n${GUI_STDERR}")
endif()

if(EXISTS "${OUTPUT_PATH}")
    file(SIZE "${OUTPUT_PATH}" OUTPUT_SIZE)
    if(NOT OUTPUT_SIZE EQUAL 0)
        message(FATAL_ERROR "GUI invalid-param regression unexpectedly produced output: ${OUTPUT_PATH}")
    endif()
endif()

if(NOT EXISTS "${SCREENSHOT_PATH}")
    message(FATAL_ERROR "GUI invalid-param regression did not produce screenshot: ${SCREENSHOT_PATH}")
endif()

file(SIZE "${SCREENSHOT_PATH}" SCREENSHOT_SIZE)
if(SCREENSHOT_SIZE EQUAL 0)
    message(FATAL_ERROR "GUI invalid-param regression produced an empty screenshot: ${SCREENSHOT_PATH}")
endif()

gis_gui_read_status_file("GUI invalid-param regression" "${STATUS_PATH}"
    STATUS_SUCCESS STATUS_CANCELLED STATUS_MESSAGE STATUS_RAW_MESSAGE)

gis_gui_assert_status_bool("${STATUS_SUCCESS}" "false"
    "GUI invalid-param regression unexpectedly reported success")
gis_gui_assert_status_bool("${STATUS_CANCELLED}" "false"
    "GUI invalid-param regression unexpectedly reported cancellation")

string(FIND "${STATUS_RAW_MESSAGE}" "failed_to_apply_param:bogus" RAW_MATCH_INDEX)
if(RAW_MATCH_INDEX EQUAL -1)
    message(FATAL_ERROR
        "GUI invalid-param regression reported unexpected raw_message.\n"
        "actual raw_message: ${STATUS_RAW_MESSAGE}\n"
        "localized message: ${STATUS_MESSAGE}")
endif()
