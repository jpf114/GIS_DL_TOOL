...
set(OUTPUT_PATH "${OUTPUT_DIR}/inference_output.tif")
set(SCREENSHOT_PATH "${OUTPUT_DIR}/inference.png")
...
execute_process(
    COMMAND "${GUI_PATH}"
        -platform offscreen
        --select-plugin inference
        --select-action inference_single
...
if(NOT "${GUI_EXIT_CODE}" STREQUAL "0")
    message(FATAL_ERROR
        "Installed GUI inference regression failed with exit code ${GUI_EXIT_CODE}\n"
        "stdout:\n${GUI_STDOUT}\n"
        "stderr:\n${GUI_STDERR}")
endif()

if(NOT EXISTS "${OUTPUT_PATH}")
    message(FATAL_ERROR "Installed GUI inference regression did not produce output: ${OUTPUT_PATH}")
endif()

file(SIZE "${OUTPUT_PATH}" OUTPUT_SIZE)
if(OUTPUT_SIZE EQUAL 0)
    message(FATAL_ERROR "Installed GUI inference regression produced an empty output file: ${OUTPUT_PATH}")
endif()

if(NOT EXISTS "${SCREENSHOT_PATH}")
    message(FATAL_ERROR "Installed GUI inference regression did not produce screenshot: ${SCREENSHOT_PATH}")
endif()

file(SIZE "${SCREENSHOT_PATH}" SCREENSHOT_SIZE)
if(SCREENSHOT_SIZE EQUAL 0)
    message(FATAL_ERROR "Installed GUI inference regression produced an empty screenshot: ${SCREENSHOT_PATH}")
endif()

gis_gui_read_status_file("Installed GUI inference regression" "${STATUS_PATH}"
    STATUS_SUCCESS STATUS_CANCELLED STATUS_MESSAGE STATUS_RAW_MESSAGE)

gis_gui_assert_status_bool("${STATUS_SUCCESS}" "true"
    "Installed GUI inference regression reported unsuccessful status")
gis_gui_assert_status_bool("${STATUS_CANCELLED}" "false"
    "Installed GUI inference regression unexpectedly reported cancellation")
